...
class PlayerArgumentType(
    private val predicate: (Player) -> Boolean,
    private val allowMultiplePlayers: Boolean,
) : ArgumentType<PlayerInput> {
    companion object {
        ...
        private val ERROR_PLAYER_NOT_MEET_CONDITION =
            SimpleCommandExceptionType(
                AdventureComponent(
                    text {
                        content("Player not meet condition")
                    }
                )
            )

        fun getSinglePlayer(context: CommandContext<*>, name: String): Player =
            context.getArgument(name, PlayerInput::class.java).players.single()

        fun getMultiplePlayers(context: CommandContext<*>, name: String): Collection<Player> =
            context.getArgument(name, PlayerInput::class.java).players
    }

    override fun parse(reader: StringReader): PlayerInput {
    ...
    private fun suggestMultiple(
        ...
        reader.cursor = builder.start
        val parser = MultipleArgumentParser(reader) { input ->
            (input.getPlayerByUuidOrName() ?: throw ERROR_PLAYER_NOT_FOUND.createWithContext(reader))
                .takeIf(predicate)
                ?: throw ERROR_PLAYER_NOT_MEET_CONDITION.createWithContext(reader)
        }
        ...
        return parser.fillSuggestions(
            builder,
            getPlayerSuggestions().map { it.name }
        )
    }
    ...
    private fun suggestPlayers(builder: SuggestionsBuilder, input: String) {
        getPlayerSuggestions().flatMap { player ->
            buildList {
                if (player.name.startsWith(input, ignoreCase = true)) {
                    add(player.name)
                }
    ...
    private fun getPlayerSuggestions(): List<Player> =
